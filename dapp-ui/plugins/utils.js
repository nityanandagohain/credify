...
let account = null
let airbnbContract
let airbnbContractAddress = '0xB570f5780B170f85aB8F384F11a6c211FDcd45e1' // Paste Contract address here
  ...
  myLoans.forEach((loan, index) => {
    ...
    for (let i = 0; i < Lenders.length; i++) {
      if (Lenders[i].loanId === loan.id) {
        // console.log('Matching Id', loan.id)
        loan_lenders = `${loan.id.keys(id)} : ${lenders}`
      }
    }
  });
